...
function listOccasion (uint _id, string memory _name, uint _cost, uint _tickets, uint _maxTickets, string memory _date, string memory _time, string memory _location, string memory _description, string memory _image) public onlyOwner {
   //the totalOccasion identifier is incremented to esnure each occasion created has a unique identifier
   totalOccasions++;

//This creates a new occasion struct and stores it in the occasions mapping. The key used for the mapping is the totalOccasions identifier which can also be used as an identifier as this number is incremented each time the function is called whenever a new occasion is created then this number should be unique for that occasion
